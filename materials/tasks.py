...
def send_update_course_mail(course_id):
    """
    Send email to subscribers if course wasn't updated more than 4 hours ago.
    """
    subscriptions = Subscription.objects.filter(course_id=course_id, status=True)
    for subscription in subscriptions:
        if (subscription.status
                and subscription.course.last_update < datetime.now(timezone.utc) + timedelta(hours=4)):
            send_mail(
